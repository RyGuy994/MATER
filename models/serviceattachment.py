...
class ServiceAttachment(Base):  # ServiceAttachment table
    ...
    )  # Service ID this goes to in class Service
    service = relationship(
        "Service", backref=backref("serviceattachments", lazy=True)
    )  # relationship to service
    attachment_path = Column(String(255))  # attachment path
    user_id = Column(
        Text, ForeignKey("user.id"), nullable=False
    )  # owner of service attachment
    service_attachment_owner = relationship(
        "User", backref=backref("service_attachment_owner", lazy=True)
