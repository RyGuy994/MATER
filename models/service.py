...
class Service(Base): # Service table
    ...
    asset_id = Column(Integer, ForeignKey('asset.id'), nullable=False) # Asset ID this goes to in Class Asset
    asset = relationship('Asset', backref=backref('services', lazy=True)) # relation to Asset
    ...
    service_owner = relationship('User', backref=backref('service_owner', lazy=True)) # relation to service
